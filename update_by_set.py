...
if not all([SUPABASE_URL, SUPABASE_KEY, ZENROWS_API_KEY]):
...
def fetch_ebay_via_proxy(search_query, player_name):
    ...
    print(f"📡 Routing ZenRows proxy query for: '{clean_query}'...")
        ...
        try:
            ...
            if not listings:
                ...
                if "captcha" in html_lower or "verify you are a human" in html_lower or "signin.ebay.com" in html_lower or "sign in to view" in html_lower:
                    print(f"  🛑 eBay Login Wall hit. Retrying with fresh ZenRows IP ({attempt+1}/3)...")
                    time.sleep(3)
                    continue
                else:
            ...
            print(f"❌ ZenRows pipeline network anomaly: {e}")
